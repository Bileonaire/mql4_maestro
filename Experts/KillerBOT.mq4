...
void OnTick()
{
  ...
  string WarmBuys = "Warm Buys = ";

  string sells = "Hot Sells = ";
  string WarmSells = "Warm Sells = ";


  // if (latestUpdateMinute != Minute() && (Minute() % 15 == 0)) {
  //   string delete_ = "https://bileonairefx.herokuapp.com/acc/delete";
  //   int send = WebRequest("POST", delete_, cookie, NULL, timeout, post, 0, result, headers);
  // }

  for(int i=0; i < length; i++)
  {
    if (MarketInfo(pairs[i], MODE_SPREAD) < 30  && IsTradeAllowed(pairs[i], TimeCurrent())) {
      double accumulated = buyStrengthPercent(pairs[i]);

      if (accumulated>12 && meetsCriteria(pairs[i], "buy")) {
        buys += pairs[i] + accumulated + " |  ";
      }
      if (accumulated<8 && meetsCriteria(pairs[i], "sell")) {
        sells += pairs[i] + accumulated + " |  ";
      }

      // if (accumulated>=15) {
      //   buys += pairs[i] +" (" + MarketInfo(pairs[i], MODE_SPREAD) +  ") - " + accumulated + " |  ";
      //   if (latestUpdateMinute != Minute() && (Minute() % 15 == 0)) SendAPIDATA(pairs[i], "buy", accumulated);
      // }
      // if (accumulated>=10 && accumulated<15) {
      //   WarmBuys += pairs[i] +" (" + MarketInfo(pairs[i], MODE_SPREAD) +  ") - " + accumulated + " |  ";
      //   if (latestUpdateMinute != Minute() && (Minute() % 15 == 0)) SendAPIDATA(pairs[i], "buy", accumulated);
      // }
      // if (accumulated<=5) {
      //   sells += pairs[i] +" (" + MarketInfo(pairs[i], MODE_SPREAD) +  ") - " + accumulated + " |  ";
      //   if (latestUpdateMinute != Minute() && (Minute() % 15 == 0)) SendAPIDATA(pairs[i], "sell", accumulated);
      // }
      // if (accumulated>5 && accumulated<10) {
      //   WarmSells += pairs[i] +" (" + MarketInfo(pairs[i], MODE_SPREAD) +  ") - " + accumulated + " |  ";
      //   if (latestUpdateMinute != Minute() && (Minute() % 15 == 0)) SendAPIDATA(pairs[i], "sell", accumulated);
      // }
    }
  }

  if (if (latestUpdateMinute != Minute() && (Minute() % 30 == 0)) {
    string chat = "-1001366966111";  // bileonaire_fx
    // sendTelegram(buys, chat);
    // sendTelegram(WarmBuys, chat);
    sendTelegram(sells, chat);
    // sendTelegram(WarmSells, chat);
    hour = Hour();
  }
...
void SendAPIDATA(string symbol, string trend, double accumulated)
{
  string url = "https://bileonairefx.herokuapp.com/acc/mt4?symbol=" + symbol + "&trend=" + trend + "&accumulate=" + accumulated+ "&time=" + TimeLocal();
  ResetLastError();
  int send = WebRequest("POST", url, cookie, NULL, timeout, post, 0, result, headers);
}

bool meetsCriteria(string cur, string direction) {
  if (environment(cur, 240) == direction && environment(cur, 15) == direction && check_prev_candle(cur, 240, 1) == direction && pop(cur, 15) == direction) return true;
  else retun false;
}
...
int mastercheck(string cur) {
  // buy
  if (check_prev_candle(cur, 43200, 1) == "buy" && PriceAboveBelowClose(cur, 43200, 1) == "above" && environment(cur, 240) == "buy" && check_prev_candle(cur, 240, 1) == "buy" && environment(cur, 15) == "buy") {
  ...
  if (check_prev_candle(cur, 43200, 1) == "sell" && PriceAboveBelowClose(cur, 43200, 1) == "below" && environment(cur, 240) == "sell" && check_prev_candle(cur, 240, 1) == "sell" && environment(cur, 15) == "sell") {
